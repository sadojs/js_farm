...
INSERT INTO users (username, password_hash, name, role, address)
VALUES ('user', '$2b$10$nzFSy4nFDGm2oP5afBzpO.r37PyM7/.7td2RrXdHfep7IzTJU3XLu', '김농부', 'farm_admin', '경기도 화성시 농업로 123');
...
FROM users u, house_groups g WHERE u.username = 'user' AND g.name = '화성 1농장';
...
FROM users u, houses h WHERE u.username = 'user' AND h.name = '1동 하우스';
...
FROM users u, houses h WHERE u.username = 'user' AND h.name = '2동 하우스';
...
WHERE u.username = 'user' AND d.tuya_device_id = 'tuya_temp_001';
...
FROM users u, devices d, generate_series(0, 23) n
WHERE u.username = 'user' AND d.tuya_device_id = 'tuya_humid_001';
...
SELECT 'user / user123 (농장 관리자)' as farm_admin_account;
