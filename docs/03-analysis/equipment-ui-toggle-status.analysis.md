...
### Groups.vue 체크박스 선택 버그 수정
- **이슈**: 장비 추가/제거 모달의 체크박스 5곳에서 `@click.stop`으로 인해 체크박스 직접 클릭 시 선택 안됨
- **수정**: 체크박스에 `pointer-events: none` + `tabindex="-1"` 적용
- **영향 범위**: 장비 추가 모달(1곳), 장비 제거 모달 — 센서(1곳), 개폐기(1곳), 관수(1곳), 일반 장비(1곳)

### 대시보드 레이아웃 변경
- **파일**: `SummaryCards.vue`
- **변경**: 4개 요약 카드(전체 장비, 활성 그룹, 자동화 룰, 온라인 기기)를 하단에서 상단(날씨 카드 아래)으로 위치 이동
